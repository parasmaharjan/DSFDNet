...
test_name = 'l1_ssim_with_added_ll_HH/'
ps = args.patch  # patch size for training
save_freq = 100
learning_rate = 1e-4
cluster = True
...
model = SFNet(args).to(device)
model.load_state_dict(torch.load(model_dir + 'l1_ssim_with_added_ll_HH/l1_ssim_with_added_ll_model_HH_sony_e1000.pth'))
opt = optim.Adam(model.parameters(), lr=learning_rate)

ssim_loss = pytorch_ssim.SSIM(window_size=11)

for epoch in range(1201, 4001):
    ...
    for ind in np.random.permutation(len(train_ids)):
        ...
        # Wavelet_decompose
        _, _, _, in_LL, in_LH, in_HL, in_HH = wd.decompose4ch(input_patch)
        _, _, _, gt4ch_LL, gt4ch_LH, gt4ch_HL, gt4ch_HH = wd.decompose4ch(gt4ch_patch)


        if 0:
            plt.figure()
            plt.subplot(221)
            plt.imshow(input_patch[0, :, :, 0], cmap="gray")
            plt.subplot(222)
            plt.imshow(gt4ch_patch[0, :, :, 0], cmap="gray")
            plt.subplot(223)
            plt.imshow(in_LH[0, :, :, 0], cmap="gray")
            plt.subplot(224)
            plt.imshow(gt4ch_LH[0, :, :, 0], cmap="gray")

        in_LL = np.minimum(in_LL, 1.0)
        # gt4ch_LH = np.maximum(gt4ch_LH, 0.0)

        # torch for input to model
        in_img = torch.from_numpy(np.concatenate((in_LL,in_HH),axis=3)).permute(0, 3, 1, 2).to(device)
        gt_img = torch.from_numpy(gt4ch_HH).permute(0, 3, 1, 2).to(device)

        model.zero_grad()
        out_img = model(in_img)

        s_loss = 1-ssim_loss(out_img, gt_img)
        ...
        #print("%d %d Loss=%.5f Time=%.5f Ratio=%.0f"%(epoch,cnt,np.mean(g_loss[np.where(g_loss)]),time.time()-st, ratio))
        print("%d %d Loss=%.5f Time=%.5f Ratio=%.0f" % (
        epoch, cnt, g_loss, time.time() - st, ratio))
        ...
        if epoch % save_freq == 0:
            ...
            if 0:
                ...
                plt.imshow(gt4ch_patch[0, :, :, 0], cmap="gray")
                plt.subplot(323)
                plt.imshow(in_HL[0, :, :, 0], cmap="gray")
                plt.subplot(324)
                plt.imshow(gt4ch_HL[0, :, :, 0], cmap="gray")
                plt.subplot(325)
            ...
            torch.save(model.state_dict(), model_dir + test_name+'l1_ssim_with_added_ll_model_HH_sony_e%04d.pth' % epoch)
